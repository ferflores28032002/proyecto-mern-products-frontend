    ...
    try {
        ...
        reader.readAsDataURL(image[0])
            ...
            reader.onloadend = async () => {
                ...
                try{
                  const data = await baseUrl.post(`/products`,{ name, description,category, price, image: reader.result} )
                  if(data.status === 200) {
  ...
  try {
      ...
      console.log(id , image)
          ...
          reader.onloadend = async () => {
              ...
              try{
                const data = await baseUrl.post(`/products/${id}`,{ image: reader.result } )
                  console.log(data)
                if(data.status === 200) {
                  toast.success("Se ha subido su imagen", { position: "top-center"})
                }
